...
namespace Framework_07
{
    public class Factory
    {
        ...
        private Factory()
        {
            int counter = Enum.GetNames(typeof(ObjectRolls)).Length;
        ...
        public GameObject createGameObject(PictureBox pictureBox, bool hasGravity, ObjectRolls roll)
        {
            charactersListCounter[(int)roll] += 1;
            return new GameObject(pictureBox, hasGravity);
        }
        // Creating Object that has Movement
        public GameObject createGameObject(PictureBox pictureBox,bool hasGravity,int speed,Movement movement,ObjectRolls roll)
        {
            charactersListCounter[(int)roll] += 1;
